...
**一键安装（终端里一条命令）**：默认装到 `~/.openclaw/workspace/skills/baby-feeding`（可用环境变量 `OPENCLAW_WORKSPACE` 改工作区根路径）。

```bash
curl -fsSL https://raw.githubusercontent.com/nannyu/baby-feeding-skill/main/scripts/install-openclaw-skill.sh | bash
```

也可把上面这一行复制到 OpenClaw 对话，让具备 `exec` 且允许联网的助手代为执行；装完后需新开会话或重启 Gateway。

**手动安装示例**（路径按你本机 workspace 调整）：
